...
def compress_and_decompress(args):
    ...
    # 这里是对datasets.py中的Evaluation类进行实例化
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames, class_name) in enumerate(tqdm(eval_loader), 0):
            data = data.to(device, dtype=torch.float)
            B = data.size(0)
            input_filenames_total.extend(filenames)
            # 创建最终保存的、含有子类别文件夹的路径
            # print(class_name[0])
            save_dir = os.path.join(args.output_dir, class_name[0])
            ...
            if args.reconstruct is True:
                ...
                if args.save is True:
                    ...
                    compression_utils.save_compressed_format(compressed_output, 
                        out_path=os.path.join(save_dir, f"{filenames[0]}_compressed.hfc"))
            ...
            if args.metrics is True:
                ...
                ssim = SSIM_func(reconstruction * max_value, data * max_value)
                PSNR_total[n:n + B] = torch.Tensor(psnr)
                # MS_SSIM_total[n:n + B] = ms_ssim.data
                SSIM_total[n:n + B] = ssim.data
            ...
            for subidx in range(reconstruction.shape[0]):
                ...
                fname = os.path.join(save_dir, "{}_RECON_{:.3f}bpp.JPEG".format(filenames[subidx], q_bpp_per_im))
    ...
    print('===========================')
    print('PSNR: {:.2f}dB'.format(df['PSNR'].mean()))
    print('SSIM: {:.4f}'.format(df['SSIM'].mean()))
    print('LPIPS: {:.4f}'.format(df['LPIPS'].mean()))
    print('Q_BPP: {:.4f}'.format(df['q_bpp'].mean()))
    print('===========================')
...
def main(**kwargs):
    ...
    input_images = glob.glob(os.path.join(args.image_dir, '**/*.JPEG'), recursive=True)
    input_images += glob.glob(os.path.join(args.image_dir, '**/*.png'), recursive=True)

    label_name = {"electric_ray": 0, "goldfish": 1, "great_white_shark": 2, "hammerhead": 3, "stingray": 4,
                  "tench": 5, "tiger_shark": 6}
    labeled_img = []
    for dir in input_images:
        # sub_dir是图片对应的子类别文件夹名
        sub_dir = os.path.basename(os.path.dirname(dir))
        # print(sub_dir)
        label_number = label_name[sub_dir]
        labeled_img.append((dir, int(label_number)))
