...
def get_dataloaders(dataset, mode='train', root=None, shuffle=True, pin_memory=True, 
    ...
    # 这里是类传递，根据传来的dataset字符串的值，将字典中映射后得到的字符串作为类名传递给Dataset
    # 在Compress中就是将Evaluation传递给了Dataset，然后Dataset就具有了Evaluation类的功能
...
class Evaluation(BaseDataset):
    ...
    def __init__(self, root=os.path.join(DIR, 'data'), normalize=False, **kwargs):
        ...
        self.imgs = glob.glob(os.path.join(root, '**/*.JPEG'), recursive=True)
        self.imgs += glob.glob(os.path.join(root, '**/*.png'), recursive=True)
    ...
    def __getitem__(self, idx):
        ...
        self.labeled_img = []
        for dir in self.imgs:
            # sub_dir是图片对应的子类别文件夹名
            sub_dir = os.path.basename(os.path.dirname(dir))
            # print(sub_dir)
            label_number = label_name[sub_dir]
            self.labeled_img.append((dir, int(label_number)))
        # 图片对应的路径和子类别序号
        img_dir, img_label = self.labeled_img[idx]
        class_name = os.path.basename(os.path.dirname(img_dir))
        ...
        return transformed, bpp, filename, class_name
...
class OpenImages(BaseDataset):
    ...
    def __init__(self, root=os.path.join(DIR, 'data/openimages'), mode='train', crop_size=256, 
        ...
        self.imgs = glob.glob(os.path.join(data_dir, '**/*.JPEG'), recursive=True)
        self.imgs += glob.glob(os.path.join(data_dir, '**/*.png'), recursive=True)
    ...
    def __getitem__(self, idx):
        ...
        # img values already between 0 and 255
        img_path = self.imgs[idx]
        _, img_label = self.labeled_img[idx]
        filesize = os.path.getsize(img_path)
        try:
